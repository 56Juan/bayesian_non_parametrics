% psbp_fd_iteracion.m — Datos reales, corrida 23 (Rio Mapocho en Los Almendros)
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 23_01_datos.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -real_nivel_v04_m03, _m04, _m05- con sus propios datos, contrato y
...
BASENAME     = "real";
SERIE_ID     = "nivel";      % variable modelada; DEBE coincidir con 23_01
VENTANA_ID   = 4;            % ventana temporal;  DEBE coincidir con 23_01
...
for M_FPCA = M_FPCA_LIST
    ...
    EXPERIMENT_ID = sprintf("%s_%s_v%02d_m%02d", ...
                            BASENAME, SERIE_ID, VENTANA_ID, M_FPCA);
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 23_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 23_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 23_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 23_01. Corre ' ...
     '23_01_datos.ipynb para cada M de M_FPCA_LIST.']);
...
for M_FPCA = M_FPCA_LIST
    eid = sprintf("%s_%s_v%02d_m%02d", BASENAME, SERIE_ID, VENTANA_ID, M_FPCA);
    p   = config_paths(eid);
    fprintf("     M=%d -> %s\n", M_FPCA, p.out_artefact);
end
fprintf("\n   Continuar con 23_03_convergencia, con el mismo M_FPCA_LIST.\n");
