% psbp_fd_iteracion.m — Datos reales, corrida 21 (Rio Mapocho en Los Almendros)
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 22_01_datos.ipynb, arma una lista plana de jobs (cadena x componente FPCA) y
% la reparte con parfor. Entrena SOLO con el bloque de entrenamiento.
%
% Identico al de las corridas de simulacion salvo por el EXPERIMENT_ID: aqui no
% hay ESCENARIO_ID ni REPLICA_ID —no hay generador ni replicas Monte Carlo—
% sino la serie modelada y la ventana temporal. Y config_paths.m apunta a
% data/reales/ y artefact/reales/ en vez de a las carpetas de simulacion.
%
% Ejes:
%   SERIE_ID     : variable modelada (nivel, caudal, ...).
%   VENTANA_ID   : ventana temporal de la serie.
%   M_FPCA       : punto del barrido en componentes FPCA.
...
BASENAME     = "real";
SERIE_ID     = "nivel";      % variable modelada; DEBE coincidir con 22_01
VENTANA_ID   = 3;            % ventana temporal;  DEBE coincidir con 22_01

M_FPCA       = 4;            % componentes FPCA; DEBE coincidir con 22_01

% EXPERIMENT_ID:
%   real_<serie>_v<ventana a 2 digitos>_m<M a 2 digitos>
% Debe coincidir EXACTAMENTE con el de 22_01_datos.ipynb. M viaja en el ID
% porque cada valor del barrido escribe sus propios artefactos y trazas.
EXPERIMENT_ID = sprintf("%s_%s_v%02d_m%02d", BASENAME, SERIE_ID, VENTANA_ID, M_FPCA);
...
assert(isfield(hp_json, "seed_base"), ...
    "hyperparameters.json no contiene 'seed_base'; regenera con 22_01.");
...
for chain = 1:N_ITER
    for k = 1:n_components
        ...
        % [ESCALERA] atau y btau salen de la entrada por componente si existe,
        % y si no del bloque global. Sin esto la mitad de la escalera se
        % ignoraria EN SILENCIO: las cuatro componentes compartirian el mismo
        % prior de tau. El respaldo mantiene compatibles los contratos
        % anteriores, que no traen atau por componente.
