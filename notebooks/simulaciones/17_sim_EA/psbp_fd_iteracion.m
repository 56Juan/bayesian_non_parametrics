% psbp_fd_iteracion.m — Escenario A (FAR(1) con tendencia determinista), corrida 17
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 17_01_simulaciones.ipynb, arma una lista plana de jobs (cadena x componente
...
%                  el Escenario A es un diagnostico propio del estudio y no un
%                  Algoritmo del anexo, de modo que se nombra con letra para que
%                  no colisione con los ids 1..6 y para que ningun barrido
%                  posterior lo confunda con una replica de aquellos.
...
%
% NOTA sobre el escenario. Los datos de entrada a este muestreador son los
% scores FPCA ESTANDARIZADOS con el bloque de entrenamiento. Bajo tendencia esa
% estandarizacion deja de ser valida en el bloque de prueba, y por construccion
% la componente que capta la deriva tiene un score de raiz cuasi-unitaria. El
% muestreador no sabe nada de esto ni tiene por que saberlo: es exactamente el
% punto del experimento. No ajustar el .m para "compensar" la tendencia.
...
ESCENARIO_ID = "A";          % escenario de diagnostico (no es Algoritmo del anexo)
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario";

% EXPERIMENT_ID incluye la replica: <basename>_<escenario>_r<replica a 2 digitos>.
% Debe coincidir EXACTAMENTE con el de 17_01_simulaciones.ipynb.
% [FIX] El formato del escenario es %s y no %d como en las corridas 11-16,
% porque aqui el id es una letra. Si se cambia a un entero, cambiar tambien el
% notebook: la coincidencia exacta de la cadena es lo que empareja los
% artefactos de ambos lados.
EXPERIMENT_ID = sprintf("%s_%s_r%02d", BASENAME, ESCENARIO_ID, REPLICA_ID);
...
assert(isfield(hp_json, "seed_base"), ...
    "hyperparameters.json no contiene 'seed_base'; regenera con 17_01.");
SEED_BASE = hp_json.seed_base;

fprintf("════════════════════════════════════════════════════════\n");
fprintf("  EXPERIMENT_ID = %s   (Escenario A: FAR(1) + tendencia)\n", EXPERIMENT_ID);
