% psbp_fd_iteracion.m — Escenario B-3 (Algoritmo B-3 del anexo), corrida 73
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 73_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_73_3_r01_m02, _m03, ...- con sus propios datos, contrato y
...
BASENAME     = "escenario_73";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 73_01,
% 73_03, 73_04 y 73_05. Cada valor tiene que estar procesado por 73_01 antes
% de correr esto.
M_FPCA_LIST  = [2 3 4 5];

% Con true, un M sin artefactos de 73_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 73_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 73_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 73_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 73_01. Corre ' ...
     '73_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 73_03_convergencia, con el mismo M_FPCA_LIST.\n");
