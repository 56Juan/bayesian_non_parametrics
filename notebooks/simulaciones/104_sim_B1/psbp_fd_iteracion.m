% psbp_fd_iteracion.m — B-1 (mezcla de mecanismos, anexo seccion B), corridas 104
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 104_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
BASENAME     = "escenario_104";

% [ENTRENAMIENTO] Solo se entrena M = max(M_FPCA_LIST) de 104_01: cada componente
% usa solo su rezago propio, asi que sus cadenas sirven a todos los M menores
% (los notebooks 104_03, 104_04 y 104_05 leen las trazas de este M).
% DEBE ser el M_ENTRENO de 104_01 y estar procesado antes de correr esto.
M_FPCA_LIST  = [4];

% Con true, un M sin artefactos de 104_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 104_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 104_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 104_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 104_01. Corre ' ...
     '104_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 104_03_convergencia (leen las trazas de este M para todo el barrido).\n");
