% psbp_fd_iteracion.m — Escenario 3 (Algoritmo A-3 del anexo), corrida 103
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 103_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_103_3_r01_m01, _m02, _m03- con sus propios datos, contrato y
...
BASENAME     = "escenario_103";

% [ENTRENAMIENTO] Solo se entrena M = max(M_FPCA_LIST) de 103_01: cada componente
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 103_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 103_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 103_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 103_01. Corre ' ...
     '103_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 103_03_convergencia (leen las trazas de este M para todo el barrido).\n");
