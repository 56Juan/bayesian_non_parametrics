% psbp_fd_iteracion.m — Escenario K (heterocedasticidad condicional en tau,
% varianza conmutada por el signo del rezago; diagnostico, no es un
% Algoritmo del anexo), corrida 34
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 34_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
% parfor. Entrena SOLO con el bloque de entrenamiento.
%
% N_LAGS = 1, igual que TODOS los demas escenarios del estudio (1-6, B, C-I):
% aunque el generador de este escenario tiene dependencia real a CUATRO
% rezagos, el diseno AR que 34_01 escribio -y que este archivo lee sin volver
% a decidir nada sobre eso- solo contiene columnas para el rezago 1. Ver la
% nota grande en 34_01_simulaciones.ipynb.
%
...
% distinto -escenario_K_1_r01_m02, _m03, _m04- con sus propios datos, contrato
% y trazas, asi que los jobs de distintos M no comparten nada mas que el pool:
% no hay riesgo de que uno pise los artefactos del otro.
%
...
ESCENARIO_ID = 1;            % placeholder numerico; el nombre del escenario va en BASENAME
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario_K";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 34_01,
% 34_03, 34_04 y 34_05. Cada valor tiene que estar procesado por 34_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2 3];

% Con true, un M sin artefactos de 34_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 34_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 34_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 34_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 34_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 34_01. Corre ' ...
     '34_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 34_03_convergencia, con el mismo M_FPCA_LIST.\n");
