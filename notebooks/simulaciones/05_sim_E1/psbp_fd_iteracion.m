...
%                  Hoy es 1; al escalar el estudio pasa a ser un eje del barrido.
%   chain        : identifica la cadena MCMC dentro de un mismo conjunto de datos.
%   k            : identifica la componente FPCA.
%
% Los nombres de archivo NO cambian: la salida sigue siendo
% chain_fpc_<idx>_iter<chain>.mat, que es lo que consume el flujo de resultados.
...
ESCENARIO_ID = 1;                      % escenario/experimento (Algoritmo del anexo)
...
%    Orden: [chain=1,k=1], [chain=1,k=2], ..., [chain=N_ITER,k=n_components]
% ════════════════════════════════════════════════════════════════════════════
...
for chain = 1:N_ITER
    for k = 1:n_components
        ...
        job.seed         = SEED_BASE + chain * 9973 + k * 31;  % semilla unica por job
        job.y            = Y_cell{k};
        job.Xnoint       = X_cell{k};
        ...
        % ruta de salida: chain_fpc_<k>_iter<chain>.mat  (nombre sin cambios)
...
fprintf("   Por job: %.2f min de pared   |   %.2f min de CPU\n", ...
        min_pared_por_job, min_cpu_por_job);
