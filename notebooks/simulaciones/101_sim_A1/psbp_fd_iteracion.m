...
% 101_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% [ENTRENAMIENTO] Solo se entrena M = max(M_FPCA_LIST) de 101_01: cada componente
% usa solo su rezago propio, asi que sus cadenas sirven a todos los M menores
% (los notebooks 101_03, 101_04 y 101_05 leen las trazas de este M).
% DEBE ser el M_ENTRENO de 101_01 y estar procesado antes de correr esto.
M_FPCA_LIST  = [8];

% Con true, un M sin artefactos de 101_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 101_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 101_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 101_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 101_01. Corre ' ...
     '101_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 101_03_convergencia (leen las trazas de este M para todo el barrido).\n");
