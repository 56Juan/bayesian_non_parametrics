% psbp_fd_iteracion.m - Escenario I (cambios de régimen de tendencia por tramos), corrida 27
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 27_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_I_r01_m01, _m02, _m03- con sus propios datos, contrato y
...
ESCENARIO_ID = "I";          % escenario de diagnostico (no es Algoritmo del anexo)
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 27_01,
% 27_03, 27_04 y 27_05. Cada valor tiene que estar procesado por 27_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2 3];
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 27_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 27_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 27_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 27_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 27_01. Corre ' ...
     '27_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 27_03_convergencia, con el mismo M_FPCA_LIST.\n");
