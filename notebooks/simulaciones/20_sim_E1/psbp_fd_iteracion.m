% psbp_fd_iteracion.m — Escenario 1 (Algoritmo 1 del anexo), corrida 20
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 20_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 20_01,
% 20_03, 20_04 y 20_05. Cada valor tiene que estar procesado por 20_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2 3];

% Con true, un M sin artefactos de 20_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 20_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(["M=%d (%s): faltan artefactos de 20_01.\n" ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 20_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ["[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d " ...
         "componentes. Regenera ese punto con 20_01."], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ["Ningun punto del barrido tiene artefactos de 20_01. Corre " ...
     "20_01_simulaciones.ipynb para cada M de M_FPCA_LIST."]);
...
if numel(unique(seeds_por_job)) < n_jobs
    fprintf(["ℹ Hay semillas repetidas ENTRE puntos del barrido (mismo " ...
...
fprintf("\n   Continuar con 20_03_convergencia, con el mismo M_FPCA_LIST.\n");
