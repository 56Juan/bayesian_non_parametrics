% psbp_fd_iteracion.m — Escenario J (cuatro rezagos, grado polinomial
% decreciente; diagnostico, no es un Algoritmo del anexo), corrida 33
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 33_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% rezagos, el diseno AR que 33_01 escribio -y que este archivo lee sin volver
% a decidir nada sobre eso- solo contiene columnas para el rezago 1. Ver la
% nota grande en 33_01_simulaciones.ipynb.
%
...
% distinto -escenario_J_1_r01_m02, _m03, _m04- con sus propios datos, contrato
...
BASENAME     = "escenario_J";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 33_01,
% 33_03, 33_04 y 33_05. Cada valor tiene que estar procesado por 33_01 antes
% de correr esto.
M_FPCA_LIST  = [2 3 4];

% Con true, un M sin artefactos de 33_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 33_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 33_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 33_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 33_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 33_01. Corre ' ...
     '33_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 33_03_convergencia, con el mismo M_FPCA_LIST.\n");
