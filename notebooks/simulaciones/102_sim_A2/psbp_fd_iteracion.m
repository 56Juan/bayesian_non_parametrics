% psbp_fd_iteracion.m — Escenario 2 (Algoritmo A-2 del anexo), corrida 102
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 102_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_102_2_r01_m01, _m02, _m03- con sus propios datos, contrato y
...
BASENAME     = "escenario_102";

% [ENTRENAMIENTO] Solo se entrena M = max(M_FPCA_LIST) de 102_01: cada componente
% usa solo su rezago propio, asi que sus cadenas sirven a todos los M menores
% (los notebooks 102_03, 102_04 y 102_05 leen las trazas de este M).
% DEBE ser el M_ENTRENO de 102_01 y estar procesado antes de correr esto.
M_FPCA_LIST  = [4];

% Con true, un M sin artefactos de 102_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 102_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 102_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 102_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 102_01. Corre ' ...
     '102_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 102_03_convergencia (leen las trazas de este M para todo el barrido).\n");
