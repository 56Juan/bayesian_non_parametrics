% psbp_fd_iteracion.m — Escenario B-2 (Algoritmo B-2 del anexo), corrida 72
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 72_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_72_2_r01_m02, _m03, ...- con sus propios datos, contrato y
...
BASENAME     = "escenario_72";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 72_01,
% 72_03, 72_04 y 72_05. Cada valor tiene que estar procesado por 72_01 antes
% de correr esto.
M_FPCA_LIST  = [2 3 4];

% Con true, un M sin artefactos de 72_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        msg = sprintf(['M=%d (%s): faltan artefactos de 72_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 72_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 72_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 72_01. Corre ' ...
     '72_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 72_03_convergencia, con el mismo M_FPCA_LIST.\n");
