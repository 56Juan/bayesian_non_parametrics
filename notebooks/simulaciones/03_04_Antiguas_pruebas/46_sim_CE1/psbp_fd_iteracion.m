...
% psbp_fd_iteracion.m — Escenario CE (composicion de estados), corrida 46
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 46_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_CE_1_r01_m01, _m02- con sus propios datos, contrato y
...
%
% Ejes del estudio:
%   ESCENARIO_ID : punto de la familia CE (1=cuatro fases, base Fourier
%                  d=11, disparo por umbral).
...
ESCENARIO_ID = 1;            % generador NUEVO: composicion de estados en
                             % cuatro fases. Los priors y el mcmc_config son
                             % los de la 45, que queda como control.
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario_CE_v1";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 46_01,
% 46_03, 46_04 y 46_05. Cada valor tiene que estar procesado por 46_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2];

% Con true, un M sin artefactos de 46_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 46_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 46_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 46_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 46_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 46_01. Corre ' ...
     '46_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 46_03_convergencia, con el mismo M_FPCA_LIST.\n");
