...
% psbp_fd_iteracion.m — Escenario L (pendiente, n_lags=3), corrida 44
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 45_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_L_4_r01_m01, _m02, _m03- con sus propios datos, contrato y
...
%
% Ejes del estudio:
%   ESCENARIO_ID : punto de la familia L (1=nivel/lag1, 2=nivel/lag3,
%                  3=pendiente/lag1, 4=pendiente/lag3).
...
ESCENARIO_ID = 5;            % generador IDENTICO al del 44 (pendiente,
                             % n_lags=3); lo que cambia es la escalera de
                             % priors por componente. El 44 es el control.
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario_L_v5";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 45_01,
% 45_03, 45_04 y 45_05. Cada valor tiene que estar procesado por 45_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2];

% Con true, un M sin artefactos de 45_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 45_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 45_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 45_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 45_01.'], ...
    ...
    for chain = 1:N_ITER
        for k = 1:n_components
            ...
            % [ESCALERA] atau y btau salen de la entrada por componente si
            % existe, y si no del bloque global. Los demas escalares siguen
            % siendo globales a proposito: la 45 varia SOLO la forma del prior
            % de tau y el de inclusion, y atar el resto al global impide que se
            % cuelen diferencias no buscadas. El respaldo mantiene la
            % compatibilidad con los contratos de las corridas 41-44, que no
            % traen atau por componente.
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 45_01. Corre ' ...
     '45_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
for M_FPCA = M_FPCA_LIST
    eid = sprintf("%s_%d_r%02d_m%02d", BASENAME, ESCENARIO_ID, REPLICA_ID, M_FPCA);
    p   = config_paths(eid);
    fprintf("     M=%d -> %s\n", M_FPCA, p.out_artefact);
end
fprintf("\n   Continuar con 45_03_convergencia, con el mismo M_FPCA_LIST.\n");
