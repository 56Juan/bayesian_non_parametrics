...
%   seed         int     Semilla RNG (única por job)
    ...
    % [FIX] randi(7) ignoraba los últimos 5 valores del rango declarado.
    betajh  = bjrange(randi(numel(bjrange), N, p));
    beta0h  = b0range(randi(numel(b0range), N, 1));
    tauh    = rgamma(atau, 1/btau, N, 1);
    Si      = randi(N, n, 1);
    alphah  = zeros(N-1, 1);
    % [v4] Arranque de psijh en la media del prior, no en cero: psijh esta
    % truncado a [0, inf) y cero es el BORDE de su soporte, de modo que con
    % mupsij > 0 pero arranque en cero la cadena queda pegada al borde y el
    % prior no se nota (medido en la corrida 40 y en la v2 del 41).
    ...
    for gt = 1:nsim
        ...
        % ── Actualizar Zil, Wil ──────────────────────────────────────────────
        % [FIX 5] El sorteo del truncado desbordaba a +-Inf y un solo Inf
        % envenenaba mu = (taumu*mumu + sum(alphah))/taumuhat, y con mu no
        % finito TODOS los alphah lo quedaban para siempre: estado absorbente.
        % Ocurria ya en la iteracion 1, porque psijh arranca en mupsij y
        % m_val = alphah - sum_j psijh|x_j - Gamma_j| escala LINEALMENTE en p.
        % Con |m_val| >~ 8.3, normcdf(-m_val) vale exactamente 1 (o 0) en doble
        % precision y norminv(1) = +Inf (norminv(0) = -Inf). Medido en la
        % corrida 41: 0 % de alphah no finitos con p<=3, 12 % con p=4, 80 % con
        % p=5 y 100 % con p=6.
        %
        % Se acotan las DOS cosas, y cada una por su motivo:
        %   - m_val a [-M_TRUNC, M_TRUNC]: mantiene el sorteo dentro de su
        %     soporte. Acotar solo la probabilidad daria valores finitos pero
        %     del lado equivocado del truncamiento, corrompiendo el muestreo.
        %     A |m_val| = 6 el probit ya vale 1e-9 o 1-1e-9: esta saturado y no
        %     se pierde informacion al recortarlo.
        %   - la probabilidad a [realmin, 1-eps/2]: garantiza que norminv sea
        %     finita pase lo que pase.
        M_TRUNC = 6;
        Q_HI    = 1 - eps/2;
        Zil = zeros(n, N);
        Wil = zeros(n, N);
        for i = 1:n
            if Si(i) < N
                for l = 1:Si(i)
                    ...
                    v_val = 1;
                    if l < Si(i)
                        q = u*normcdf((0-m_val)/sqrt(v_val),0,1);
                        Zil(i,l) = m_val + sqrt(v_val)*norminv(min(max(q, realmin), Q_HI),0,1);
                    elseif l == Si(i)
                        q = u+(1-u)*normcdf((0-m_val)/sqrt(v_val),0,1);
                        Zil(i,l) = m_val + sqrt(v_val)*norminv(min(max(q, realmin), Q_HI),0,1);
                    end
        ...
        for h = 1:N-1
            ...
            for j = 1:p
                if gammajh(h,j) == 0
                    ...
                    u     = rand(1,1);
                    % [FIX 6] Mismo desborde que [FIX 5], pero aqui v_val NO es
                    % 1 (vale ~1/850), de modo que lo que satura no es m_val
                    % sino el punto de truncamiento ESTANDARIZADO
                    % a_val = -m_val/sqrt(v_val). La guarda original
                    % (psijh == inf -> 0.01) atrapaba el +Inf pero metia un
                    % valor arbitrario, y no cubria NaN ni -Inf.
                    %
                    % Se acota a_val SOLO POR ARRIBA. Recortarlo tambien por
                    % abajo pondria un techo de M_TRUNC*sqrt(v_val) ~ 0.2 a
                    % psijh, que es justo la cola que el gating necesita para
                    % discriminar: se cargaria el mecanismo que se quiere medir.
                    % Por arriba el recorte solo actua cuando la posterior pide
                    % psijh << 0, que el truncamiento manda a ~0 igual.
