% psbp_fd_iteracion.m — Escenario 1 (Algoritmo 1 del anexo), corrida 40
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 40_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
BASENAME     = "escenario_40";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 40_01,
% 40_03, 40_04 y 40_05. Cada valor tiene que estar procesado por 40_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2 3];

% Con true, un M sin artefactos de 40_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 40_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 40_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 40_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 40_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 40_01. Corre ' ...
     '40_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 40_03_convergencia, con el mismo M_FPCA_LIST.\n");
