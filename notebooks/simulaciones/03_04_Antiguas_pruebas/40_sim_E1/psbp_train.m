...
%   seed         int     Semilla RNG (única por job)
    ...
    Si      = randi(N, n, 1);
    alphah  = zeros(N-1, 1);
    % [CORRIDA 40] Arranque de psijh en la media del prior, no en cero.
    % psijh esta truncado a [0, inf) y cero es el BORDE de su soporte: con
    % mupsij = 0 la cadena arranca pegada al borde y ahi se queda, que es el
    % gating apagado medido en las corridas 41 y 43. El codigo original de
    % Chung & Dunson (references/Case1_01.m:62) deja comentada justamente una
    % alternativa de este tipo (%psijh=1/3*ones(N-1,p)).
