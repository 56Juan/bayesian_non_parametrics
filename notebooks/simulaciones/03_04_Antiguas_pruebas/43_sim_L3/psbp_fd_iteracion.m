% psbp_fd_iteracion.m — Escenario L (pendiente, n_lags=1), corrida 43
%
% Paso 2 del ciclo Python -> MATLAB -> Python. Lee el contrato que escribio
% 43_01_simulaciones.ipynb, arma una lista plana de jobs y la reparte con
...
% distinto -escenario_L_3_r01_m01, _m02, _m03- con sus propios datos, contrato y
...
ESCENARIO_ID = 3;            % Escenario L: pendiente, n_lags=1
REPLICA_ID   = 1;            % replica Monte Carlo
BASENAME     = "escenario_L_v4";

% [BARRIDO] componentes FPCA. DEBE coincidir con el M_FPCA_LIST de 43_01,
% 43_03, 43_04 y 43_05. Cada valor tiene que estar procesado por 43_01 antes
% de correr esto.
M_FPCA_LIST  = [1 2];

% Con true, un M sin artefactos de 43_01 se SALTA con aviso en vez de abortar
...
for M_FPCA = M_FPCA_LIST
    ...
    % Debe coincidir EXACTAMENTE con el de 43_01_simulaciones.ipynb. M viaja en
    ...
    if ~isfile(hp_path) || ~isfile(manifest_path)
        ...
        msg = sprintf(['M=%d (%s): faltan artefactos de 43_01.\n' ...
    ...
    assert(isfield(hp_json, "seed_base"), ...
        "[M=%d] hyperparameters.json no contiene 'seed_base'; regenera con 43_01.", ...
        M_FPCA);
    ...
    assert(n_components == M_FPCA, ...
        ['[M=%d] %s declara M=%d pero su hyperparameters.json tiene %d ' ...
         'componentes. Regenera ese punto con 43_01.'], ...
...
n_jobs = numel(jobs);
assert(n_jobs > 0, ...
    ['Ningun punto del barrido tiene artefactos de 43_01. Corre ' ...
     '43_01_simulaciones.ipynb para cada M de M_FPCA_LIST.']);
...
fprintf("\n   Continuar con 43_03_convergencia, con el mismo M_FPCA_LIST.\n");
