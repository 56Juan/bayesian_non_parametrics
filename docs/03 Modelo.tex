...
Por tanto, la brecha de investigación que este trabajo busca cubrir consiste en desarrollar un modelo bayesiano no paramétrico que opere directamente sobre los pesos de la formulación del stick-breaking. En concreto, la curva $X_t(\tau)$ se proyecta sobre la base de componentes principales funcionales y el vector de scores resultante, $\boldsymbol{\xi}_t\in\mathbb{R}^M$, se modela mediante una mezcla probit stick-breaking cuyos pesos dependen del estado rezagado. \\


 
\section{Probit Stick-Breaking Process Mixture}
\label{03_03_00_psbp}
Esta sección presenta el Probit Stick-Breaking Process (PSBP) y el modelo de mezcla construido sobre él, el PSBP Mixture (PSBPM), desarrollado por \citet{03_02_chung2009nonparametric}. Salvo indicación en contrario, las definiciones, resultados y el esquema de inferencia expuestos en esta secciónprovienen de dicho trabajo.\\
...
El presente estudio de simulación evalúa el desempeño predictivo del PSBPM-FD en dos ejes complementarios. El primero examina el comportamiento interno del modelo, comparando su rendimiento a medida que se incorpora un número creciente de scores de la representación funcional, con el fin de analizar cómo la dimensión de la representación incide en la calidad de las predicciones.\\

El segundo eje contrasta el desempeño del PSBPM-FD frente a los siguientes modelos de referencia:

\begin{itemize}
    \item Modelo autorregresivo funcional (FAR): El modelo se ajusta sobre los  coeficientes de la representación de las curvas.
    \item Random Forest y XGBoost: Ajustados sobre los scores, aumentando el número de scores utilizados de manera análoga al primer eje, de modo que la comparación con el PSBPM-FD se realice bajo la misma dimensión de  representación.
\end{itemize}

Para este fin, se realizan simulaciones bajo distintos escenarios, en las que el desempeño se evalúa de dos maneras. Por un lado, la calidad de las predicciones sobre las curvas se mide, para el PSBPM-FD, el FAR, Random Forest y XGBoost, mediante las métricas de error presentadas en la Sección~\ref{02_02_03_02_metricas_error}. Por otro lado, dado que solo el FAR y el PSBPM-FD permiten construir intervalos, la evaluación de la incertidumbre se restringe a estos dos modelos y se realiza exclusivamente a través de las medidas de evaluación descritas en la Sección~\ref{02_02_03_04_calibracion}.
