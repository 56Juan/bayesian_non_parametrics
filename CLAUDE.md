...
- **Predictiva funcional** (`models/pspb_fd_v3`): `PSBPPredictor` → `muestrear_scores` → `PropagadorFuncional` → muestras `(S, n, G)`. `ruta_traza`, `leer_traza` y `ModeloTraza` (envoltorio con `.traces` crudo + matriz de diseño con intercepto) viven en `psbp_fd_v3.py` desde la corrida 30: antes cada notebook (`30_04`, `30_05`, y probablemente 11–27) redefinía su propia copia de la convención `chain_fpc_<idx>_iter<NN>.mat`, y `leer_traza` local no normalizaba la forma de `beta0hout`/`tauhout` que sí normaliza `cargar_trazas_mat`. `curva_media_desde_scores(scores_std, fpca, estandarizador)` es el mapa determinista (`modo_residuo="ninguno"` sin muestreo) que necesitan los competidores de predicción puntual para entrar en la misma escala de curva que el PSBPM-FD.
- **Métricas**: `fit/metrics_puntual` (RMSE, MSE/R² por coeficiente, MISE, razón de dispersión) y `fit/metrics_distribucional` (CRPS muestral, energy score, cobertura, PIT muestral, LPS). Cubren el eje 1 y la parte marginal del eje 2. Desde la corrida 30, `fit/metrics_distribucional.py` distingue **cobertura puntual** (`indicador_cobertura`, promedio sobre τ, la de siempre) de **cobertura simultánea** (`indicador_cobertura_simultanea`, un AND sobre τ: 1 sólo si la curva ENTERA cae dentro de la banda). `ventana_movil_funcional` expone la segunda como `picp_simultaneo`, siempre `<= picp` por construcción.
...
**`30_04`/`30_05` divergieron del patrón `NN_04_evaluacion`/`NN_05_comparacion` de las demás corridas, y no hay que tomarlos como plantilla para 11–27.** Dos cambios de fondo, decididos en la propia corrida 30 y no propagados hacia atrás:

- **`30_04` ya no tiene ninguna tabla agregada train/test entero.** Las 7 métricas del eje 1 —MAE, RMSE, error máximo (`linf_medio`, el máximo real por curva promediado sobre la ventana, no un cuantil), Winkler, cobertura simultánea, cobertura puntual (PICP), MPIW— se calculan **una sola vez**, sobre la ventana móvil, y todo lo demás (tabla resumen con mínimo/máximo/promedio, comparación entre `M`, "qué `M` gana cada ventana") se deriva de esa misma tabla. Se eliminaron `Q_EXTREMO` (alimentaba `q95_abs`, que dejó de mostrarse en cualquier figura al pasar el error máximo a `linf_medio`) y el bootstrap de bloques de las cifras agregadas (no sobrevivió el paso a "todo es ventana móvil"; si hace falta banda de incertidumbre sobre un % de ventanas ganadas o una razón, está pendiente reincorporarla ahí). Las PIP (antes sólo en `_03`) se agregaron como §7 de `30_04`, reusando el mismo `matriz_pip`/`contraste_con_verdad` y el mismo `VERDAD` (todas las covariables activas, Escenario 1).
- **`30_05` estima el FAR sobre los COEFICIENTES B-spline, no sobre la curva en la grilla.** Es un cambio metodológico, no de implementación: antes el `FAR1` de `far_operador.py` se ajustaba con `X` `(T, L)` en la grilla, pesos trapezoidales. Ahora se blanquea `THETA` (los coeficientes que persistió `_01`) con la Cholesky de la Gram de la base (`W = LL^T`, `theta_w = THETA @ L`), de modo que el producto escalar euclídeo de `theta_w` **es** el producto `L^2` exacto (verificado con `assert` contra `theta' W theta`), y el `FAR1` se ajusta ahí con `pesos="conteo"` —pesar de nuevo con la trapezoidal sería aplicar la métrica `L^2` dos veces—. El tope de `kn` pasa a ser `K` (el número de funciones base elegido por GCV en `_01`, 4–12 según el escenario) en vez de `L=75`. Añade un eje de comparación que las corridas 11–27 no tienen: FAR en el espacio de la base, PSBPM-FD/RF/GBT en el de los `M` scores FPCA.

Bloque B (Winkler/coberturas/MPIW) en `30_05` sigue restringido a FAR y PSBPM-FD —los únicos con un mecanismo de intervalo propio—, con tres filas por `M`: la banda nativa de cada uno y una banda gaussiana del PSBPM-FD con el mismo mecanismo del FAR, para separar "gana por el modelo" de "gana por la forma de la predictiva".
